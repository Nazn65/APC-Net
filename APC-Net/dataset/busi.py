...
class BUSIDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img_s1, img_s2, ignore_mask = deepcopy(img), deepcopy(img), deepcopy(img)
        ...
        if random.random() < 0.8:
            ignore_mask  = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(ignore_mask)
        ignore_mask = blur(ignore_mask, p=0.5)
        ignore_mask = torch.from_numpy(np.array(ignore_mask)).unsqueeze(0).float() / 255.0

        return img, img_s1, img_s2, ignore_mask, cutmix_box1, cutmix_box2
