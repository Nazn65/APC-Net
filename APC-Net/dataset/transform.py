...
def normalize(img, mask):
    ...
    if img.ndimension() == 3 and img.size(0) == 1:  # Check if it's a single-channel image
...
def resize(img, mask, ratio_range):
    ...
    if not isinstance(img, PIL.Image.Image):
        raise TypeError("img 必须是 PIL.Image 对象")
    w, h = img.size
...
def blur(img, p=0.5):
    if isinstance(img, torch.Tensor):
        img = transforms.ToPILImage()(img)  # 转换 Tensor 为 PIL.Image
    if random.random() < p:
